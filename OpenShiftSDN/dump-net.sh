...
for i in $PODS ; do
    ...
    oc rsh -n openshift-sdn -c sdn pod/${POD} ovs-ofctl -O OpenFlow13 --no-stats dump-flows br0 | perl -pe 's/(reg0|reg1)=([^ ,]+)/sprintf("%s=%d",$1, hex($2))/eg' > ${DUMP_FILE}.${NODE}.OpenFlow13
    echo "Write: ${DUMP_FILE}.${NODE}.iptables"
    oc rsh -n openshift-sdn -c sdn pod/${POD} iptables-save > ${DUMP_FILE}.${NODE}.iptables
done;
